        ...
        if (isNoteworthy && entry.lastRateLimitSig !== sig) {
          ...
          if (resetsAt) {
            try {
              ...
              const ms = d.getTime();
              const now = Date.now();
              // Sanity: a real reset time is within now-1d to now+7d. Anything outside
              // that window (e.g. the upstream-bug 1970 placeholder) is suppressed
              // to avoid misleading the user.
              if (!Number.isNaN(ms) && ms > now - 86400000 && ms < now + 7 * 86400000) {
                parts.push(`重置时间：${d.toLocaleString()}`);
              }
            } catch {}
          }
          wsSendR(entry, { type: 'system_message', message: parts.join('\n'), kind: 'rate-limit' });
