    ...
    switch (event.type) {
      case 'system':
        ...
        if (event.subtype === 'init') {
          ...
          wsSend(entry.ws, { type: 'system_message', message: parts.join('\n'), kind: 'init' });
          ...
          if (event.summary && typeof event.summary === 'string') {
            wsSend(entry.ws, { type: 'system_message', message: event.summary, kind: 'compact-summary' });
          }
        } else if (event.subtype === 'error' && event.message) {
          wsSend(entry.ws, { type: 'system_message', message: String(event.message), kind: 'error' });
        ...
        if (isNoteworthy && entry.lastRateLimitSig !== sig) {
          ...
          if (resetsAt) {
            try {
              ...
              if (!Number.isNaN(d.getTime())) parts.push(`重置时间：${d.toLocaleString()}`);
            } catch {}
          }
          wsSend(entry.ws, { type: 'system_message', message: parts.join('\n'), kind: 'rate-limit' });
        }
      ...
      case 'assistant': {
        ...
        for (const block of content) {
          if (block.type === 'text' && block.text) {
            ...
            wsSend(entry.ws, { type: 'thinking_delta', text: thinkingText }, true);
      ...
      case 'result': {
        ...
        if (event.subtype && event.subtype !== 'success' && entry.ws) {
          ...
          wsSend(entry.ws, { type: 'system_message', message: msg, kind: 'error' });
        }
        break;
      }
    }
  ...
  function processCodexEvent(entry, event, sessionId) {
    ...
    switch (event.type) {
      ...
      case 'item.failed': {
        ...
        if (!item.id) {
          if (entry.ws) wsSend(entry.ws, { type: 'system_message', message: errMsg, kind: 'error' });
