function createAgentRuntime(deps) {
    ...
    if (payload && payload.type === 'system_message' && entry.sessionId && typeof loadSession === 'function' && typeof saveSession === 'function') {
      try {
        ...
        if (sess) {
          ...
          // R41: persist init-card structured detail so historical sessions
          // can re-render the expandable MCP / tools / slash-commands view.
          if (payload.initDetail) persisted.initDetail = payload.initDetail;
          // R42: persist error-class so historical error bubbles get the same
          // typed coloring as live ones.
          if (payload.errorClass) persisted.errorClass = payload.errorClass;
          // R43: persist hookEvent so historical hook bubbles use the right icon.
          if (payload.hookEvent) persisted.hookEvent = payload.hookEvent;
          // R49: persist warningType for model_fallback / context_warning banners.
          if (payload.warningType) persisted.warningType = payload.warningType;
          sess.messages.push(persisted);
          saveSession(sess);
        }
      } catch {}
    }
  ...
  // R42: classify a raw error string into one of six visual classes so the
  // client can paint the correct color/icon. Conservative regex set —
  // unknown is the safe fallback. Mirrors the categories formatRuntimeError
  // distinguishes (server.js:1490) but exposes them as a tag, not as text.
  function classifyRuntimeError(raw) {
    if (!raw) return 'unknown';
    const s = String(raw).toLowerCase();
    if (/authentication|unauthorized|forbidden|invalid api key|invalid_api_key|credential|login required/.test(s)) return 'auth';
    if (/rate limit|rate_limit|quota|billing|credits|too many requests|429/.test(s)) return 'rate-limit';
    if (/overloaded|overload_error|service unavailable|503|capacity/.test(s)) return 'overload';
    if (/context|too long|max[_ ]tokens|window.*exceed|400.*token/.test(s)) return 'context-overflow';
    if (/bad request|400|invalid_request|invalid argument/.test(s)) return 'bad-request';
    if (/network|timed out|timeout|econnreset|enotfound|etimedout|tls|certificate|fetch failed|disconnect/.test(s)) return 'network';
    return 'unknown';
  }
  ...
  function ensureCodexToolCall(entry, item) {
    ...
    tc = {
      ...
      startedAt: Date.now(), // R47: history replay elapsed
    };
  ...
  // R51: shared tool_result handler used by both case 'user' (live SDK path)
  // and case 'assistant' (defensive — some adapters may still embed it).
  // Also merges toolUseResult top-level enrichment (stdout/stderr/interrupted
  // /isImage/noOutputExpected) when present.
  function processToolResultBlock(entry, block, toolUseResult) {
    const images = [];
    let resultText;
    ...
    if (tc) {
      ...
      if (tc.startedAt && !tc.elapsedMs) tc.elapsedMs = Date.now() - tc.startedAt;
      // R51: stash toolUseResult enrichment so the bubble can show
      // stdout/stderr/interrupted/exit-code/isImage when available.
      if (toolUseResult && typeof toolUseResult === 'object') {
        const enrich = {};
        if (typeof toolUseResult.stdout === 'string') enrich.stdout = toolUseResult.stdout;
        if (typeof toolUseResult.stderr === 'string') enrich.stderr = toolUseResult.stderr;
        if (typeof toolUseResult.interrupted === 'boolean') enrich.interrupted = toolUseResult.interrupted;
        if (typeof toolUseResult.exitCode === 'number') enrich.exitCode = toolUseResult.exitCode;
        if (typeof toolUseResult.isImage === 'boolean') enrich.isImage = toolUseResult.isImage;
        if (Object.keys(enrich).length) tc.toolUseResult = enrich;
      }
    }
    const payload = {
      type: 'tool_end',
    ...
    if (tc?.toolUseResult) payload.toolUseResult = tc.toolUseResult;
    ...
    switch (event.type) {
      case 'system':
        ...
        if (event.subtype === 'init') {
          const toolsArr = Array.isArray(event.tools) ? event.tools : [];
          const mcpArr = Array.isArray(event.mcp_servers) ? event.mcp_servers : [];
          const slashArr = Array.isArray(event.slash_commands) ? event.slash_commands : [];
          ...
          // R41: structured init detail for expandable init-card UI.
          const initDetail = {
            model: event.model || null,
            cwd: event.cwd || null,
            apiKeySource: event.apiKeySource || event.api_key_source || null,
            outputStyle: event.outputStyle || event.output_style || null,
            permissionMode: permMode,
            mcpServers: mcpArr.map((s) => ({
              name: (s && (s.name || s.server_name)) || '',
              status: s?.status || (s?.connected ? 'connected' : 'unknown'),
              error: s?.error || s?.last_error || null,
            })),
            tools: toolsArr.map((t) => (typeof t === 'string' ? t : (t?.name || ''))).filter(Boolean),
            slashCommands: slashArr.map((c) => ({
              name: typeof c === 'string' ? c : (c?.name || ''),
              isCustom: !!(c && (c.is_custom || c.custom)),
            })).filter((c) => c.name),
          };
          wsSendR(entry, { type: 'system_message', message: parts.join('\n'), kind: 'init', initDetail });
        ...
        } else if (event.subtype === 'model_fallback') {
          // R49: Anthropic-side model fallback (e.g. opus → sonnet on overload).
          // CLI shows this as a yellow banner; cc-web previously dropped it
          // entirely. Format: '模型降级：<original> → <fallback>[ · <reason>]'.
          const orig = event.original_model || event.from_model || event.from || '';
          const fb = event.fallback_model || event.to_model || event.to || event.model || '';
          const reason = event.reason || event.message || '';
          const parts = [];
          parts.push(`模型降级${orig && fb ? `：${orig} → ${fb}` : ''}`);
          if (reason) parts.push(reason);
          wsSendR(entry, { type: 'system_message', message: parts.join(' · '), kind: 'warning', warningType: 'model_fallback' });
        } else if (event.subtype === 'context_warning') {
          // R49: CLI emits this when remaining context drops below ~10%.
          const usedTokens = event.used_tokens || event.used || 0;
          const totalTokens = event.context_window || event.total || 0;
          const pct = totalTokens ? Math.round((usedTokens / totalTokens) * 100) : null;
          const msg = pct != null
            ? `上下文剩余不足：${usedTokens.toLocaleString?.() || usedTokens} / ${totalTokens.toLocaleString?.() || totalTokens} tokens（${pct}%）— 建议尽快 /compact`
            : (event.message || '上下文剩余不足，建议尽快 /compact');
          wsSendR(entry, { type: 'system_message', message: msg, kind: 'warning', warningType: 'context_warning' });
          ...
          if (isNoteworthy) {
            ...
            // R43: pass hookEvent so client can render the right icon/color
            // per CLI's hook category (PreToolUse / PostToolUse / Stop /
            // UserPromptSubmit / Notification / SessionStart / PreCompact).
            wsSendR(entry, { type: 'system_message', message: parts.join('\n'), kind: 'hook', hookEvent: hookName });
          }
        }
        break;
      ...
      case 'user': {
        // R51: Claude SDK delivers tool_result blocks inside type='user' events
        // (CLI message-flow convention), not inside type='assistant'. cc-web's
        // switch previously had no 'user' case, so every tool result was
        // silently dropped — tool chips stayed in 'in-progress' forever and
        // the result text never reached the bubble. This is the root cause
        // behind all stored sessions having toolCalls with done:false /
        // result:null.
        const userContent = event.message?.content;
        if (!Array.isArray(userContent)) break;
        for (const block of userContent) {
          if (block?.type === 'tool_result') {
            processToolResultBlock(entry, block, event.toolUseResult);
          }
        }
        break;
      }
      ...
      case 'assistant': {
        ...
        for (const block of content) {
          if (block.type === 'text' && block.text) {
            ...
            // R53: accumulate fullThinking so handleAssistantDone can persist
            // it onto the assistant msg — historical reload was losing the
            // entire 'thinking process' details panel.
            if (entry.fullThinking != null) entry.fullThinking += thinkingText;
            wsSendR(entry, { type: 'thinking_delta', text: thinkingText, sessionId }, true);
          } else if (block.type === 'redacted_thinking') {
            // R44: surface CLI's '[encrypted reasoning]' placeholder. Anthropic
            // sometimes redacts a thinking block server-side; previously cc-web
            // silently dropped it.
            const placeholder = '[已加密的推理段，模型已对此次思考做加密处理]\n';
            if (entry.fullThinking != null) entry.fullThinking += placeholder;
            wsSendR(entry, { type: 'thinking_delta', text: placeholder, sessionId, redacted: true }, true);
            ...
            if (FILE_TOOLS.includes(block.name)) {
              ...
              // R45: MCP tool names follow mcp__<server>__<action>. Split so the
              // chip can show 'mcp:server' badge + action label distinctly,
              // matching how the CLI surfaces MCP-origin tools.
              ...
              meta = {
                kind: 'mcp_tool_call',
                title: action || block.name,
                subtitle: server ? `mcp:${server}` : '',
                mcpServer: server,
                mcpAction: action,
              };
            } else if (block.name === 'ExitPlanMode') {
              // R44: ExitPlanMode is the CLI's plan-approval handoff. Render it
              // as a distinct proposal card instead of a generic tool chip.
              kind = 'plan_proposal';
              meta = { kind: 'plan_proposal', title: '计划提案', plan: toolInput?.plan || '' };
            } else if (block.name === 'Task') {
              // R44: Task tool spawns a sub-agent. Tag with sub_agent kind so
              // the client can render a distinct outline (full nesting will
              // come once SDK parent_tool_use_id is verified live).
              kind = 'sub_agent';
              meta = {
                kind: 'sub_agent',
                title: 'Sub-Agent · ' + (toolInput?.subagent_type || 'general-purpose'),
                description: typeof toolInput?.description === 'string' ? toolInput.description : '',
                prompt: typeof toolInput?.prompt === 'string' ? toolInput.prompt.slice(0, 240) : '',
              };
            }
            // R47: stamp startedAt so history replay can compute elapsed.
            const tc = { name: block.name, id: block.id, input: toolInput, done: false, kind, meta, startedAt: Date.now() };
            if (entry.toolCalls.length < MAX_TOOL_CALLS) entry.toolCalls.push(tc);
            else entry.toolCallsTruncated = true;
            ...
            // R51: in practice tool_result blocks arrive inside type='user'
            // events. This branch is defensive in case any adapter wraps them
      ...
      case 'result': {
        // R43: surface non-trivial stop_reason as a bubble-foot chip so the
        // user immediately sees the reply was truncated/refused/paused
        // (rather than buried inside the ctx-meter popover).
        if (event.stop_reason && event.stop_reason !== 'end_turn' && event.stop_reason !== 'tool_use') {
          entry.lastStopReason = event.stop_reason;
          wsSendR(entry, {
            type: 'assistant_stop',
        ...
        if (session) {
          ...
          const u = event.usage;
          if (u) {
            session.totalUsage = session.totalUsage || { inputTokens: 0, cachedInputTokens: 0, outputTokens: 0, cacheCreationTokens: 0, cacheReadTokens: 0 };
            session.totalUsage.inputTokens += (u.input_tokens || 0);
            session.totalUsage.outputTokens += (u.output_tokens || 0);
            // R40: keep cache_creation and cache_read split so the UI can show
            // both. cachedInputTokens stays for backward-compat aggregation.
            session.totalUsage.cacheCreationTokens = (session.totalUsage.cacheCreationTokens || 0) + (u.cache_creation_input_tokens || 0);
            session.totalUsage.cacheReadTokens = (session.totalUsage.cacheReadTokens || 0) + (u.cache_read_input_tokens || 0);
            session.totalUsage.cachedInputTokens += (u.cache_read_input_tokens || 0) + (u.cache_creation_input_tokens || 0);
            entry.lastUsage = u;
          }
          // R40: capture context window from modelUsage (CLI status-line shows
          // "Context: N / MAX" — this is the data source).
          if (event.modelUsage && typeof event.modelUsage === 'object') {
            const firstModel = Object.values(event.modelUsage)[0];
            if (firstModel?.contextWindow && (!session.contextWindow || firstModel.contextWindow > session.contextWindow)) {
              session.contextWindow = firstModel.contextWindow;
            }
            if (firstModel?.maxOutputTokens) {
              session.maxOutputTokens = firstModel.maxOutputTokens;
            }
          }
          saveSession(session);
        }
        ...
        // R40: rich per-turn metrics (TTFT, durations, turn count, context fill,
        // permission denials, service tier, etc.). Used to drive the ctx-meter
        // and the hover popover.
        // R47: also persist this payload on entry.lastUsageDetail so
        // handleAssistantDone can attach it to the assistant message — without
        // this the ctx-popover's drill-down data is purely ephemeral and any
        // page reload loses the per-turn metrics for historical assistants.
        // R50: compute detail UNCONDITIONALLY (was nested inside if(entry.ws)),
        // otherwise a ws drop during the result event would skip the
        // lastUsageDetail assignment too, losing the persisted drill-down.
        // wsSendR remains conditional — buffer-replay handles the rare case
        // where ws was alive when result arrived but the frame was lost.
        {
          const u = event.usage || {};
          const turnInput = (u.input_tokens || 0) + (u.cache_creation_input_tokens || 0) + (u.cache_read_input_tokens || 0);
          const detail = {
            type: 'usage_detail',
            turnUsage: {
            ...
            maxOutputTokens: session?.maxOutputTokens || null,
            contextUsed: turnInput,
            ...
            // R46: forward the full denial array (tool_name, tool_use_id, tool_input)
            // instead of just length. CLI shows the full list so users see WHICH
            // tool the model wanted to call and what input was rejected.
            permissionDenials: Array.isArray(event.permission_denials)
              ? event.permission_denials.map((d) => ({
                  toolName: d?.tool_name || '',
                  toolUseId: d?.tool_use_id || '',
                  toolInput: d?.tool_input || null,
                }))
              : [],
            serviceTier: event.service_tier || null,
            stopReason: event.stop_reason || null,
          };
          entry.lastUsageDetail = detail;
          // wsSendR is itself ws-aware (buffers when entry.ws is null), so
          // call it unconditionally — the buffer-replay path will flush on
          // reconnect if needed (R49 added usage_detail to REPLAYABLE_TYPES).
          wsSendR(entry, detail, true);
        }
  ...
  function processCodexEvent(entry, event, sessionId) {
    ...
    switch (event.type) {
      ...
      case 'item.completed': {
        ...
        tc.resultTotalLength = truncated.totalLength;
        tc.isError = isError;
        if (tc.startedAt && !tc.elapsedMs) tc.elapsedMs = Date.now() - tc.startedAt; // R47
        wsSendR(entry, {
          type: 'tool_end',
          toolUseId: item.id,
          result: truncated.text,
      ...
      case 'turn.failed': {
        ...
        // R42: surface the failure immediately so the user sees a typed error
        // bubble instead of a silent spinner until handleProcessComplete fires
        // ~5-30s later. Skip when the user just aborted (avoids double-banner).
        if (entry.ws && !entry.aborted) {
          wsSendR(entry, {
            type: 'system_message',
            message,
            kind: 'error',
            errorClass: classifyRuntimeError(message),
          });
        }
        break;
      }
      ...
      case 'error':
        if (event.message) {
          if (/^Reconnecting\.\.\./.test(event.message)) {
            ...
            // R42: same as turn.failed — surface non-reconnect errors so the
            // chat doesn't silently freeze until handleProcessComplete.
            if (entry.ws && !entry.aborted) {
              wsSendR(entry, {
                type: 'system_message',
                message: event.message,
                kind: 'error',
                errorClass: classifyRuntimeError(event.message),
              });
            }
          }
        }
