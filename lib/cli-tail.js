...
// R63: `onOffsetAdvance(offset)` fires every time the tail consumes more of
// the file. The caller persists the latest offset to session.lastJsonlOffset
// so a subsequent attach after ws disconnect resumes from there instead of
// jumping to EOF and silently dropping all the rows written during the gap.
function createTail({ jsonlPath, onEvent, onError, onClose, onOffsetAdvance, startFromEnd = false, startOffset = null }) {
  let offset = 0;
  ...
  function readFromOffset() {
    ...
    fs.stat(jsonlPath, (err, stat) => {
      ...
      stream.on('end', () => {
        ...
        inFlight = false;
        // Report the advanced offset (minus any partial line still buffered)
        // so the caller can checkpoint resume position.
        try { onOffsetAdvance?.(offset - Buffer.byteLength(lineBuf, 'utf8')); } catch {}
      });
    });
  }
